...
private void print_details()
  ...
  for (i = 0; i < spec.row_map.Length; ++i)
    {
    System.Console.Write("State " + i.ToString());
...
private void States()
  ...
  for (int index = 0; index < spec.state_dtrans.Length; ++index)
    {
    outstream.Write("  " + spec.state_dtrans[index].ToString());
...
private void RMap_table()
  ...
  for (int i = 0; i < size; ++i)
    {
    outstream.Write(spec.row_map[i].ToString());
    if (i < lastelem)
      outstream.Write(",");
    if (((i + 1) % 8) == 0)
      outstream.Write("\n  ");
...
;
  ...
  outstream.Write(hdr_str
    ...
    + "          if (yy_last_accept_state < " +
          spec.accept_list.Count.ToString()
		  + ")\n"
