...
static private void computeClasses(Spec spec)
...
#if DEBUG
	    Console.WriteLine("Adding char "+ nextcls - 1.ToString()+" split="+ split.ToString()+" i="+ i.ToString());
#endif
	    }
	  ccls[i] = (int) h[split];
	  }
  ...
  Console.WriteLine();
  Console.WriteLine("NFA has "+ nextcls.ToString()+" distinct character classes.");
