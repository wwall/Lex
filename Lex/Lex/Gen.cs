...
#endif
      nfa = (Nfa) o;
      Console.Write(spec.nfa_states.IndexOf(nfa).ToString() + " ");
      }
...
public int Advance()
  ...
  if (ibuf.line_index > ibuf.line_read)
    {
    Console.WriteLine("ibuf.line_index = " + ibuf.line_index.ToString());
    Console.WriteLine("ibuf.line_read = " + ibuf.line_read.ToString());
    Utility.assert(ibuf.line_index <= ibuf.line_read);
    }
...
private void print_header()
  ...
  foreach (string state in spec.states.Keys)
    ...
    if (DTrans.F != spec.state_dtrans[index])
      ...
      Console.WriteLine("\tStart index in transition table: "
			+ spec.state_dtrans[index].ToString());
      }
    else
      {
  ...
  for (int i = 0; i < spec.dtrans_list.Count; ++i)
	...
	{
	Console.Write(" * State " + i.ToString() + " [nonaccepting]");
	}
      else
	{
	Console.Write(" * State " + i.ToString()
		      + " [accepting, line "
		      + dtrans.GetAccept().line_number.ToString()
		      + " <"
	...
	if (last_transition != dtrans.GetDTrans(j))
	  {
	  Console.Write("\n *    goto " + dtrans.GetDTrans(j).ToString() + " on ");
	  chars_printed = 0;
	  }
